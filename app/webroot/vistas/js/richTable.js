...
richTable.pages = new (function() {
	...
	function createFooter(table) {
		...
		// First link
		var a = createLink(table, "inicio", "not-number first disabled", -1);
		td.appendChild(a);
		
		// Previous link
		var a = createLink(table, "ant.", "not-number previous disabled", -1);
		td.appendChild(a);
		...
		// Next link
		var a = createLink(table, "sig.", length > 1 ? "not-number next" : "not-number next disabled", length > 1 ? 2 : -1);
		td.appendChild(a);		
		
		// Last link
		var a = createLink(table, "fin", length > 1 ? "not-number last" : "not-number last disabled", length > 1 ? length : -1);
		td.appendChild(a);
	}
